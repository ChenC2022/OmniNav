...
- **首页**：**常用**（置顶书签，无数量上限；展示区固定高度、超出可滚动；通过分类内书签右键「添加到常用」添加；编辑布局下通过把手拖拽排序；深色模式专用配色）、**分类卡片**（书签按分类展示，支持私密分类密码解锁；编辑布局下分类名前把手可拖拽排序、书签项把手可跨分类拖拽；书签图标中号、状态检测小圆点位于图标右上角；宽屏自适应多列 / 可选 CSS 多列瀑布流；**删除分类时该分类下书签会移入未分类**）、**未分类**（独立区域，在分类下方：图标按钮添加 / 导入 / 清理失效 / 自动归类，书签网格自适应列数）、检查链接、新建/编辑分类与书签
