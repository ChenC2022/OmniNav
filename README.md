...
- **聚合搜索**：位于**顶部栏**（仅首页显示），支持**网页搜索**与**书签搜索**；**默认书签搜索**，有输入时才显示书签结果下拉。点击「网页/书签」标签或 Tab 键切换模式；点击弹层外任意区域或按 Escape 关闭下拉并退出书签模式。**超窄屏**下搜索会折叠为放大镜按钮，点击后弹出搜索面板并自动聚焦。
...
- **顶部信息条**：时间为主，日期与天气在副信息中轮播展示；**窄屏**下自动隐藏以优先保证搜索与关键操作按钮。天气数据由 Open‑Meteo 提供并在边缘缓存（15 分钟）。
