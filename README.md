...
- **技术栈**：Vue 3 + Vite + Tailwind CSS v4 + Pinia + VueUse + vuedraggable；API：Hono on Cloudflare Pages Functions；存储：KV
- **文档**：需求见 [docs/OmniNav PRD.md](docs/OmniNav%20PRD.md)，开发计划见 [docs/DEVELOPMENT_PLAN.md](docs/DEVELOPMENT_PLAN.md)
...
- **全局**：顶部栏（城市 + 时间 + 天气、聚合搜索（网页 / `@` 书签）、⌘K 聚焦书签搜索、AI、设置）、右侧 AI 抽屉对话、底部 Footer（主题切换、同步状态）
...
将输出的 `id` 填入 `wrangler.toml` 中 `[[kv_namespaces]]` 的 `id` 字段（替换 `REPLACE_WITH_YOUR_KV_NAMESPACE_ID`）。
...
生产环境 KV 需在 Cloudflare 控制台为该项目绑定同名 `KV_OMNINAV`。
