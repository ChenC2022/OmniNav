...
### 3. 配置主人密码（必须，单用户对外暴露时）

未配置时访问站点会进入登录页，但无法登录（提示「未配置主人密码」），所有受保护接口均不可用。
...
- 点击 **Add variable** → **Encrypt**（作为 Secret）
  ...
  - **Value**：你的主人密码（如 `test123`，建议生产环境使用强密码）
- 选择 **Production**（及 Preview 如需要）后保存

保存后需**重新部署**（Deployments → 最新部署 → Retry deployment）使配置生效。
...
若 API 返回 503 且提示 "KV not available"，请回到 **方式 A 的步骤 4** 在 Dashboard 中为该 Pages 项目绑定同名 `KV_OMNINAV`（CLI 部署时若未自动带上绑定，需在 Dashboard 补绑）。
...
在项目根目录创建 `.dev.vars`（已加入 .gitignore），内容一行：`OMNINAV_OWNER_PASSWORD=你的密码`。不创建则本地请求不鉴权，可直接访问。生产环境配置见上方 **步骤 3**。
