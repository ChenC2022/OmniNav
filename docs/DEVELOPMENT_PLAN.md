...
### Phase 11：UI 精细化与 AI 快速添加 ✅

- **UI 布局重构** ✅：
  - 「拖动书签」开关移动至顶部栏（AppHeader），带垂直分隔线，全局共享 `isEditLayout` 状态。
  - 分类卡片（CategoryCard）与未分类区域（Home）的操作按钮全部移至左侧标题旁，操作重心更统一。
  - 常用区（Pinned）网格自适应优化，适配移动端窄屏显示。
  - 分类卡片支持 **4 级折叠**，持久化存储 `viewLevel` 状态。
  - **细节打磨**：分类右上角书签统计；未分类折叠逻辑保护（minViewLevel）；AI 对话新会话刷新功能。
  - **回到顶部**：新增 `BackToTop` 悬浮组件，支持透明磨砂质感，监听「常用区域」动态阈值。
  - **书签编辑增强**：书签表单支持手动选择分类与 AI 自动建议分类（联动选中）。
- **AI 快速添加** ✅：
  - 在常用区与分类区之间添加 `QuickPasteBar` 快速粘贴栏组件（`src/components/quick-add/QuickPasteBar.vue`）。
  - 支持粘贴单个或多个 URL（多行/逗号/空格分隔），自动解析并去重。
  - 可选「深度分析」开关：抓取目标网页摘要（`/api/extract-meta`），提高 AI 归类与标题生成准确率。
  - AI 自动为每条链接生成标题、描述并建议归入现有分类或创建新分类（`/api/ai/chat`）。
  - 结果确认弹窗：用户可逐条修改分类归属（下拉选择含现有分类、AI 建议新分类、未分类、跳过），底栏实时显示将被添加的数量。
  - 确认后一键批量创建书签与新分类，同步至 KV；统一书签表单按键配色为 Indigo。
  - 已有 URL 去重：粘贴已存在的链接时直接提示无需重复添加。
